...
diff  = double(rgb2gray(I)) * 255 - I_rec;
diff  = diff / max(max(diff));
diff  = im2uint8(diff);
I_rec = I_rec / max(max(I_rec));
I_rec = im2uint8(I_rec);

subplot(2,2,1); imshow(I_rec, [0,2^n-1]);
subplot(2,2,2); imshow(diff, [0 2^n-1])
subplot(2,2,3); imhist(I_rec);
subplot(2,2,4); imhist(diff);
